...
Entry* listContainsEntry(MyList *list, void *key, int(*equal)(void *key1, void *key2))
{
	MyListIterator *it;
	Entry *entry;

	it = createMyListIterator(list);
	while (myListIteratorHasNext(it)) {
		entry = (Entry *)(myListIteratorNext(it));
...
void rebuildMyHashMap(MyHashMap *map)
{
	...
	MyList **newentryList;
	MyHashMapEntryIterator* it;
	Entry *entry;

	newentryList = (MyList **) malloc(sizeof(MyList*) * newSize);
	for (i = 0; i < newSize; i++)
		newentryList[i] = createMyList();

	it = createMyHashMapEntryIterator(map);
	while (myHashMapEntryIteratorHasNext(it)) {
		entry = myHashMapEntryIteratorNext(it);
		hasCode = (*(map->hashCode))(entry->key);
		hasCode %= newSize;
		if (hasCode < 0)
			hasCode += newSize;
...
MyHashMap *createMyHashMap(int(*hashCode)(void *key), int(*equal)(void *key1, void *key2))
{
	...
	re = (MyHashMap *)malloc(sizeof(MyHashMap));
	re->size = 0;
	...
	for (i = 0; i < re->initialCapacity; i++)
...
//使用全部参数创建HashMap
MyHashMap *createMyHashMapForAll(int initialCapacity, float loadFactor, int(*hashCode)(void *key),
				int(*equal)(void *key1, void *key2))
{
	MyHashMap *re;

	re = createMyHashMap(hashCode, equal);
...
int myHashMapContainsKey(MyHashMap *const map, void *const key)
{
	...
	if (hasCode < 0)
		hasCode += map->initialCapacity;
	re = listContainsEntry(map->entryList[hasCode], key, map->equal);
...
{
	MyHashMapEntryIterator *re;

	re = (MyHashMapEntryIterator*) malloc(sizeof(MyHashMapEntryIterator));
	re->count = 0;
	re->index = 0;
	re->map = map;
...
Entry *myHashMapEntryIteratorNext(MyHashMapEntryIterator *iterator)
{
	...
	re = (Entry *)iterator->current->data;
...
int myHashMapRemoveDataByKey(MyHashMap *const map, void *const key)
{
	...
	it = createMyListIterator(map->entryList[hasCode]);
	re = 0;
